...
def create_model(learn_rate=0.1, epochs = 150, batch_size=32, optimizer="rmsprop",init="glorot_uniform"):
...
# model_cv = GridSearchCV(estimator=model, param_grid=params, n_jobs=-1, cv=3, verbose=2).fit(X_train, y_train)

# print("Best: %f using %s" % (model_cv.best_score_, model_cv.best_params_))

tuned_model = create_model(batch_size=10, epochs=50, init='normal', learn_rate=1e-07, optimizer='rmsprop')
